...
try:
    ...
    import json as json
from edl.taxon import ranks as rank_oader
from jme.jupy_tools import smart_labels
...
def convertToNx(
    counts,
    leaves=True,
    ranks=["superkingdom", "class", "genus"],
    colorMap=None,
    **kwargs
):
    """
    ...
    if ranks is None:
        raise Exception(
            "This does not work unless you specify a rank. Try going straing to JSON with convertTaxToJSON()"
        )
    ...
    G.nodes[root][valueKey] = counts[root]

    ranks = sorted(ranks, key=rank_order.index, reverse=False)
        ...
        while True:
            ...
            if parent.rank in ranks or nodeIsLeaf or parent is root:
                if parent not in used:
                    ...
                    G.nodes[parent][valueKey] = counts.get(parent, 0)
                    logger.debug(
                        "Adding parent: %s (%d)" % (parent, G.nodes[parent][valueKey])
                    )
                    breakAfter = False
                    used.add(parent)
                else:
                ...
                if parent is not node:
                    logger.debug(
                        "adding counts to %s (%d + %d)"
                        % (parent, G.nodes[parent][valueKey], countSum)
                    )
                    G.nodes[parent][valueKey] = (
                        G.nodes[parent].get(valueKey, 0) + countSum
                    )
...
def _add_node(graph, taxNode, colorMap, nameKey, colorKey):
    ...
    name = taxNode.name
    graph.nodes[taxNode][nameKey] = nameCleanRE.sub("", name)
            ...
            if key.isAncestorOf(taxNode):
                if colorMapKey is None:
                    ...
                    if colorMapKey.isAncestorOf(key):
                        colorMapKey = key
        graph.nodes[taxNode][colorKey] = colorMap[colorMapKey]
...
def _plotArc(tree, node, ax, total, sort, level=1, theta=0, usedNodes=None, **kwargs):
    ...
    nodeSum += tree.nodes[node].get(valueKey, 0)
    width = 2.0 * pi * nodeSum / total
    color = tree.nodes[node].get(colorKey, "r")
...
def getChildren(node, tree, sortType):
    kids = tree.neighbors(node)
    if sortType is None:
        return kids
    else:
        return sorted(kids, key=lambda n: tree.nodes[n].get(sortType, n))
...
def clip_string(name, name_width=13):
    if len(name) > name_width:
        # remove middle and insert elipsis to get to 13 characters
        start_end = int(np.ceil((name_width - 3) / 2))
        end_start = int(np.ceil((3 - name_width) / 2))
...
def plotSunburstJSON(
    ...
    **kwargs
    ...
    optional arguments:
        ...
        clip_labels: limit labels to this many characters
        use_smart_labels: 
            if True: experimental feature to prevent label overlaps
        label_kwargs: kwargs passed to smart_labels.annotate_points
                         or ax.annotate
    """ % (
        VALUE,
        VALUE,
    ...
    ptotal, labels = plotArcJSON(tree, ax, total, sort, **kwargs)

    label_kwargs.setdefault("ha", "center")
    label_kwargs.setdefault("va", "center")

    if clip_labels > 0:
        labels = [(clip_string(l[0], clip_labels),) + l[1:] for l in labels]

    if use_smart_labels:
        # draw labels and adjust if they overlap
        smart_labels.annotate_points(ax, labels, axis="x", **label_kwargs)
    else:
        # just draw them in the given location
        annots = [
            smart_labels._annotate_point(ax, a_tuple, **label_kwargs)
...
def plotArcJSON(tree, ax, total, sort, level=1, theta=0, usedNodes=None, **kwargs):
    ...
    # children start where this node starts
    childTheta = theta
    kids = 0

    # text and location of labels for this node and children
    labels = []
    ...
    for child in _getJSONChildren(tree, kwargs.get(KIDS, KIDS), sort):
        ...
        if level > 40:
            ...
            raise Exception("Too deep!")
        child_end, child_labels = plotArcJSON(
            child,
            ax,
            total,
            sort,
        ...
        # keep track of children's total
        nodeSum += child_end

        # next childe starts at end of this child
        childTheta += 2.0 * pi * child_end / total
    ...
    # total of this node plus children
    nodeValue = tree.get(valueKey, 0)
    nodeSum += nodeValue
    logger.debug("value for %s is %d (%d)", tree.get(NAME, "?"), nodeValue, nodeSum)

    width = 2.0 * pi * nodeSum / total
    center = theta + (width / 2.0)

    color = tree.get(colorKey, "r")

    # plot the bar (arc) for this node
    logger.debug("plotting: x=%f, w=%f, theta=%f", center, width, theta)
    rects = ax.bar([center,], [1,], width=width, bottom=level, color=color, **kwargs)
    ...
    if nodeValue > 0 or kids > 1:
        ...
        if polar:
            ...
            rotation = 90
        labels.append((tree.get(NAME, ""), (x, y), {"rotation": rotation}))
    return nodeSum, labels
...
def applyCutoff(tree, cutoff, **kwargs):
    ...
    group_smols: if True, replace all small children in one parent with a single 
    node that represents their sum
    """
...
def cutoffTree(tree, cutoff, **kwargs):
    ...
    for child in tree.get(kidsKey, []):
        ...
        if childSum >= cutoff:
            ...
            smol_kids_sum += childSum
            smol_kids_count += 1

    if kwargs.get("group_smols", False):
        if smol_kids_sum > 0:
            childrenToKeep.append(
                {valueKey: smol_kids_sum, nameKey: "Others (%d)" % (smol_kids_count),}
            )
    else:
        tree[valueKey] = tree.get(valueKey, 0) + smol_kids_sum
...
def testNXConversion():
    ...
    (nxtreeo, rooto) = convertToNx(
        testCounts, ranks=["superkingdom", "phylum", "order"]
    )
    myAssertEq(usum, nxSum(nxtreeo))
    logger.debug("..with species")
    (nxtreeos, rootos) = convertToNx(
        testCounts, ranks=["superkingdom", "phylum", "order", "species"]
    )
